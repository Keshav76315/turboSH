...
| **13.2** | **State-Transition Dynamics (LSTM, Transformer, GNN)** | "Could extend to LSTM; current ensemble"       | **LSTMForecaster** (`forecasting/models/lstm_model.py`) 2-layer stacked LSTM with multi-step output head ($t+1, t+2, t+3$). Trained weights (`models/forecasting/lstm_model.pt`) and ONNX export (`models/forecasting/forecast_lstm.onnx`) with dynamic batch/seq axes. HMM and Markov baselines also implemented. **Gap:** Live ONNX proxy serving slated for V5. | 🟡 **Partial** (Trained models & ONNX exported, proxy serving in V5) |
| **13.3** | **Forecast Future States & Attacker Progression**      | "Does this partially; predict next 5 requests" | **Multi-step forecast ($t+1, t+2, t+3$)** via `LSTMForecaster`, `HMMForecaster`, and `MarkovChain`. Evaluated with early-warning lead-time analysis (~12.5s advance notice before surge). Benchmark documented in `docs/forecast_evaluation_v3.md`. **Gap:** Live sliding window buffer and proxy trigger slated for V5. | 🟡 **Partial** (Multi-horizon benchmarked, live proxy serving in V5) |
| **13.4** | **Map Behaviour to MITRE ATT&CK Stages**               | "Current feature patterns map to MITRE stages" | `forecasting/mitre_mapping.py` maps 5 attack stages to 12 MITRE ATT&CK techniques (T1595, T1498, T1499, etc.) with `map_forecast_to_mitre()` timeline API. **13 unit tests passing.** **Gap:** Not surfaced in dashboard or API responses. | 🟡 **Partial** (Code & tests exist, no UI/API exposure) |
| **13.5** | **Explainability (Attention, Feature Attribution)**    | "Each score comes with feature breakdown"      | **Explainer** (`forecasting/explainability.py`) implements native autograd Input x Gradient saliency mapping (<2ms) across 6 telemetry features. Generates `ForecastExplanation` with percentage feature attributions and automated natural language reasoning mapped to MITRE ATT&CK. | 🟢 **Implemented** (Gradient saliency, feature attributions, MITRE reasoning) |
