...
| **13.2** | **State-Transition Dynamics (LSTM, Transformer, GNN)** | "Could extend to LSTM; current ensemble"       | **MarkovChain** (`forecasting/models/markov_chain.py`) and **Gaussian HMM** (`forecasting/models/hmm_model.py`) fitted with Viterbi decoding. **Gap:** Models are offline artifacts; not yet called from Go runtime inference path. No LSTM/GNN. | 🟡 **Partial** (Offline models exist, no live serving) |
| **13.3** | **Forecast Future States & Attacker Progression**      | "Does this partially; predict next 5 requests" | **3-step forecast** via `HMMForecaster.forecast()` and `MarkovChain.predict_trajectory()`. `AttackProgressionModel` computes escalation/de-escalation probabilities. **Gap:** Not integrated into Go proxy runtime; forecasts are offline/CLI only. | 🟡 **Partial** (Offline forecasting, no live integration) |
