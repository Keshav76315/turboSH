...
**Generated**: 2026-09-17 19:11:28 UTC  
**Architecture**: 2-Layer Stacked LSTM + Multi-Horizon Dense Head  
**Training Duration**: 13.87 seconds  
...
- **Benchmark Observation**: In this benchmark on synthetic telemetry, the LSTM serves as an initial deep temporal baseline and currently trails both the Markov Chain and Gaussian HMM baselines across all horizons (t+1: HMM & Markov tie for lead, t+2: HMM & Markov tie for lead, t+3: HMM & Markov tie for lead).
- **t+1 Accuracy Comparison**: LSTM 64.33% vs HMM 66.25% (-1.92%), Markov: 66.25% (-1.92%) — HMM & Markov tie for lead
- **t+2 Accuracy Comparison**: LSTM 46.83% vs HMM 50.00% (-3.17%), Markov: 50.00% (-3.17%) — HMM & Markov tie for lead
- **t+3 Accuracy Comparison**: LSTM 44.35% vs HMM 46.01% (-1.66%), Markov: 46.01% (-1.66%) — HMM & Markov tie for lead
...
- **Production Export**: Fully validated ONNX model ready for sub-millisecond Go reverse proxy inference (parity diff: 6.83e-05).
...
| `models/forecasting/forecast_lstm.onnx` | Exported ONNX compute graph | Zero-Python runtime serving | ✅ Validated (diff: 6.83e-05) |
