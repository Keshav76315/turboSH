"""
Unit tests for Transformer ONNX export and runtime numerical parity (forecasting/export/export_transformer_onnx.py).
"""
...
class TestTransformerONNXExport:
    """Test suite for Transformer ONNX export, checker validation, and runtime inference parity."""
    ...
    def test_export_and_parity_validation(self, model):
        with tempfile.TemporaryDirectory() as tmpdir:
            onnx_path = os.path.join(tmpdir, "test_transformer.onnx")

            exported_path = export_transformer_to_onnx(
                model=model,
    ...
    def test_dynamic_axes_support(self, model):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            export_transformer_to_onnx(model=model, output_path=onnx_path, seq_len=10)
    ...
    def test_training_mode_preserved_during_export_and_validation(self, model):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            export_transformer_to_onnx(model=model, output_path=onnx_path, seq_len=10)
            assert model.training is True, "export_transformer_to_onnx must preserve model.training state"
